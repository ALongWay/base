...
NSString *const LevelDBRootFile = @"levelDBFiles";
...
+(id)databaseInDocumentWithName:(NSString *)name
{
    LevelDBOptions opts = MakeLevelDBOptions();
    return [self databaseInDocumentWithName:name andOptions:opts];
}

+ (id)databaseInDocumentWithName:(NSString *)name andOptions:(LevelDBOptions)opts
{
    NSString *path = [getDocumentPath() stringByAppendingPathComponent:name];
    LevelDB *ldb = [[[self alloc] initWithPath:path name:name andOptions:opts] autorelease];
    return ldb;
}
